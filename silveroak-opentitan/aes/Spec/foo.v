...
Require Import Coq.Lists.List Coq.Bool.Bool Coq.Init.Specif.
...
Section Spec.
  Definition state : Type := list (list bool).
  Definition key : Type := list bool.

  (* zip two lists together, returning a list of the length of the left list, if b is
     shorter, it is imagined to be extended with copies of default *)
  Fixpoint zip_left (t : Type) (default : t) (a : list t) (b : list t) : list (t * t) :=
    match a with
    | nil => nil
    | h :: hs => (h, (hd default b)) :: zip_left t default hs (tl b)
    end.

  Definition xor_pair (p : bool * bool) := let (a, b) := p in xorb a b.

  Definition xor_vec (a : list bool) (b : list bool) : list bool :=
    map xor_pair (zip_left bool false a b).

  Lemma inverse_xorb (b1 : bool) (b2 : bool) : xorb (xorb b1 b2) b2 = b1.
    induction b1; induction b2; simpl; reflexivity.
  Qed.

  Lemma inverse_xor_vec (a : list bool) (b : list bool) : xor_vec (xor_vec a b) b = a.
   
  Admitted.

  Definition add_round_key (input : state) (k : key) :=
    map (fun column => xor_vec column k) input.

  Lemma inverse_add_round_key (x : state) (k : key) :
  ...
  Proof.
    ...
    simpl.
    rewrite IHx.
    rewrite inverse_xor_vec.
    reflexivity.
  Qed.

  Context (sub_bytes shift_rows mix_columns : state -> state)
...
Lemma tail_eq (t : Type) (a : t) (l1 : list t) (l2 : list t) : l1 = l2 <-> a::l1 = a::l2.
    split.
    intros.
    rewrite H.
    reflexivity.
